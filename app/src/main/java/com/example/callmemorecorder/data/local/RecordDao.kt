...
class RecordDao(private val db: AppDatabase) {
    ...
    init {
        scope.launch { refreshFlow() }
    }

    private suspend fun refreshFlow() {
        _recordsFlow.value = queryAll()
    }
    ...
    suspend fun updateUploadStatus(id: String, status: String) = withContext(Dispatchers.IO) {
        ...
        db.writableDatabase.update(AppDatabase.TABLE_RECORDS, cv,
            "${AppDatabase.COL_ID} = ?", arrayOf(id))
        refreshFlow()
    }
    ...
    private fun Cursor.toEntity() = RecordEntity(
        id = getString(getColumnIndexOrThrow(AppDatabase.COL_ID)),
        title = getString(getColumnIndexOrThrow(AppDatabase.COL_TITLE)),
        createdAt = getLong(getColumnIndexOrThrow(AppDatabase.COL_CREATED_AT)),
        durationMs = getLong(getColumnIndexOrThrow(AppDatabase.COL_DURATION_MS)),
        localPath = getString(getColumnIndexOrThrow(AppDatabase.COL_LOCAL_PATH)),
        mimeType = getString(getColumnIndexOrThrow(AppDatabase.COL_MIME_TYPE)) ?: "audio/mp4",
        status = getString(getColumnIndexOrThrow(AppDatabase.COL_STATUS)) ?: "IDLE",
        uploadStatus = getString(getColumnIndexOrThrow(AppDatabase.COL_UPLOAD_STATUS)) ?: "NOT_STARTED",
        transcriptionStatus = getString(getColumnIndexOrThrow(AppDatabase.COL_TRANSCRIPTION_STATUS)) ?: "NOT_STARTED",
        driveFileId = getString(getColumnIndexOrThrow(AppDatabase.COL_DRIVE_FILE_ID)),
        driveWebLink = getString(getColumnIndexOrThrow(AppDatabase.COL_DRIVE_WEB_LINK)),
        transcriptText = getString(getColumnIndexOrThrow(AppDatabase.COL_TRANSCRIPT_TEXT)),
        errorMessage = getString(getColumnIndexOrThrow(AppDatabase.COL_ERROR_MESSAGE)),
        updatedAt = getLong(getColumnIndexOrThrow(AppDatabase.COL_UPDATED_AT))
    )
